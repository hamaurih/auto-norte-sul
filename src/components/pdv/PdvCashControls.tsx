...
export function PdvCashControls({
  ...
  const [type, setType] =
    useState<PosCashMovementType>("withdrawal");
  ...
  async function submitMovement() {
    ...
    if (
      ...
      toast.error(
        "Informe valor e motivo com pelo menos 3 caracteres",
      );
      return;
    }
  ...
  async function submitClose() {
    const pending =
      await countPendingPosSales().catch(
        () => 0,
      );

    if (pending > 0) {
      toast.error(
        `Existem ${pending} venda${pending === 1 ? "" : "s"} de contingência pendente${pending === 1 ? "" : "s"}. Sincronize antes de fechar o caixa.`,
      );
      return;
    }
    ...
    if (
      ...
      toast.error(
        "O fechamento do caixa exige conexão para conferir os valores no servidor.",
      );
      return;
    }
    ...
    try {
      ...
      const difference = Number(
        result?.difference_amount ?? 0,
      );

      toast.success(
        `Caixa fechado · diferença: R$ ${difference.toFixed(2).replace(".", ",")}`,
      );
      onClosed();
    } catch (error) {
      toast.error(
