...
export function AdminModuleCard({ module }: { module: AdminModule }) {
  ...
  return (
    <section className="group/card flex flex-col overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm transition-all hover:-translate-y-0.5 hover:border-foreground/15 hover:shadow-lg">
      <div className="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-3 border-b border-border/60 p-5">
        <span className={`grid size-11 shrink-0 place-items-center rounded-xl ${module.accent}`}>
      ...
      <ul id={panelId} className="flex flex-col gap-1.5 p-3">
        {shortcuts.map((shortcut) => (
          <li key={`${module.key}-${shortcut.to}-${shortcut.label}`}>
            <Link
              to={shortcut.to}
              className="group flex min-h-11 items-center gap-3 rounded-xl px-3 py-2 text-sm transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
      ...
      {hidden > 0 && (
        <button
          ...
          className="mx-3 mb-3 flex min-h-11 items-center justify-center gap-1.5 rounded-xl border border-border/70 px-3 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
