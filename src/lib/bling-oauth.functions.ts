...
function validateRedirectUri(value: string): string {
  ...
  if (url.protocol !== "https:" || url.pathname !== CALLBACK_PATH || !ALLOWED_CALLBACK_HOSTS.has(url.hostname)) {
...
export const getSecureBlingAuthUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: cfg, error: cfgError } = await (context.supabase as any)
      ...
      .limit(1)
      .single();
    if (cfgError || !cfg?.id) throw new Error(cfgError?.message ?? "Configuração Bling não encontrada.");
    ...
    // Short retention and one-time state. Only the hash is stored in the database.
