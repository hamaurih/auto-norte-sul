...
async function assertRoles(sb: any, userId: string, tenantId: string, roles: string[]) {
  const { data, error } = await sb
    ...
    .eq("active", true)
    .maybeSingle();
  if (error || !data?.role || !roles.includes(String(data.role))) throw new Error("Forbidden");
}
...
export const upsertAlias = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertRoles(context.supabase, context.userId, context.tenantId, ["owner", "admin", "manager"]);
    const row = {
    ...
    const { data: ins, error } = await context.supabase
      .from("search_aliases")
      .upsert(row, { onConflict: "tenant_id,normalized_term,target_type,target_slug" })
      .select("id")
      .single();
    if (error) throw new Error(error.message);
...
export const listNoResultLogs = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    // Agrupar por termo normalizado apenas dentro do tenant ativo.
