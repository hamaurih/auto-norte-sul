...
function AuthPage() {
  ...
  async function activeTenantRole(userId: string): Promise<string | null> {
  ...
  async function redirectAfterAuth(userId: string) {
    ...
    const role = await activeTenantRole(userId);
    if (role === "owner" || role === "admin" || role === "manager") navigate({ to: "/admin" });
    else if (role === "sales") navigate({ to: "/vendedor" });
    else navigate({ to: "/" });
  }

  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session?.user) void redirectAfterAuth(data.session.user.id);
    });
  }, []);
    ...
    <div className="grid min-h-screen md:grid-cols-2">
      <div className="relative hidden bg-gradient-to-br from-secondary to-primary md:block">
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-10 text-white">
          ...
          <div>
            ...
            <p className="mt-2 max-w-sm text-sm opacity-80">Acesso ao catálogo completo, preço atacado (B2B) e ecossistema Norte Sul.</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center bg-background p-6">
        <div className="w-full max-w-sm">
          ...
          <h1 className="font-display text-2xl font-bold uppercase">{mode === "login" ? "Entrar" : "Criar conta"}</h1>
          <p className="text-sm text-muted-foreground">{mode === "login" ? "Bem-vindo(a) de volta." : "Cadastro rápido para comprar."}</p>

          <button onClick={google} className="mt-4 flex w-full items-center justify-center gap-2 rounded-md border border-border bg-card px-4 py-2.5 text-sm font-semibold hover:bg-muted">
          ...
          <form onSubmit={submit} className="space-y-3">
            {mode === "signup" && <label className="block"><span className="mb-1 block text-xs font-semibold uppercase">Nome</span><input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" /></label>}
          ...
          <p className="mt-4 text-center text-sm">{mode === "login" ? <>Não tem conta?{" "}<button onClick={() => setMode("signup")} className="font-semibold text-primary hover:underline">Cadastre-se</button></> : <>Já tem conta?{" "}<button onClick={() => setMode("login")} className="font-semibold text-primary hover:underline">Entrar</button></>}</p>
        </div>
      </div>
    </div>
  );
