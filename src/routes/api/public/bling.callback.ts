  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        // Atomic consume: a state can be used only once and only before expiry.
        ...
        if (stateError || !stateRow?.config_id || !stateRow?.redirect_uri) {
          return html(400, "Autorização expirada", "Esta autorização é inválida, já foi utilizada ou expirou. Inicie uma nova conexão pelo painel.");
        }
        ...
        try {
          ...
          if (!tokRes.ok || !tokenPayload.access_token) {
            ...
            return html(502, "Falha na autorização", "O Bling não devolveu um access_token válido. Inicie uma nova autorização pelo painel.");
          }
