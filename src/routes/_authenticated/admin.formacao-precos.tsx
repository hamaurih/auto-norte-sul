...
function ProductRuleRow({ product, globalMarkup, onSaved }: { product: any; globalMarkup: number; onSaved: () => void }) {
  ...
  const pricing = product.pricing ?? {}; const cost = Number(product.average_cost ?? 0); const variable = Number(pricing.tax_rate ?? 0)+Number(pricing.commission_rate ?? 0)+Number(pricing.payment_fee_rate ?? 0)+Number(pricing.other_variable_rate ?? 0); const fixed = Number(pricing.fixed_cost_per_unit ?? 0); const desired = Number(pricing.desired_margin_rate ?? .3); const minimum = cost>0 && variable<1 ? (cost+fixed)/(1-variable) : null; const recommended = cost>0 && variable+desired<1 ? (cost+fixed)/(1-variable-desired) : null; const current = Number(product.price_b2c ?? 0); const unsafe = minimum != null && current>0 && current<minimum;

  return <div className={"grid gap-3 rounded-2xl border bg-white p-4 lg:grid-cols-[minmax(0,1fr)_130px_160px_130px_110px] lg:items-center "+(unsafe?"border-rose-300":"border-slate-200")}>
    <div className="min-w-0"><p className="truncate font-semibold text-slate-900">{product.name}</p><p className="mt-1 text-xs text-slate-500">{product.internal_code || product.sku} · B2B {money(product.price_b2b)} · B2C atual {money(product.price_b2c)}</p>{cost>0?<p className={"mt-1 text-xs font-semibold "+(unsafe?"text-rose-700":"text-emerald-700")}>Custo real {money(cost)} · mínimo {minimum == null ? "—" : money(minimum)} · recomendado {recommended == null ? "—" : money(recommended)}{unsafe?" · abaixo do mínimo":""}</p>:<p className="mt-1 text-xs font-semibold text-amber-700">Sem custo real aprovado</p>}</div>
...
function PricingCenterPage() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 pb-16">
    <header className="rounded-3xl bg-gradient-to-r from-slate-950 via-slate-900 to-blue-950 p-6 text-white shadow-sm lg:p-8"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10"><BadgeDollarSign className="h-6 w-6" /></div><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-200">Gestão comercial</p><h1 className="mt-1 text-3xl font-extrabold">Formação e reajuste de preços</h1><p className="mt-1 text-sm text-slate-300">Preço atual, mínimo e recomendado com base no custo real e nos custos de venda.</p></div></div></header>
