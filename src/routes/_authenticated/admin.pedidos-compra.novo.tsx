...
function NovoPedidoCompraPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: (result) => {
      ...
      void navigate({ to: "/admin/pedidos-compra/$id", params: { id: result.id } });
    },
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold uppercase">Novo pedido de compra</h1>
      ...
      <section className="rounded-lg border border-border bg-card p-4">
        ...
        <div className="mt-3 space-y-2">
          ...
          {items.map((item, index) => (
            <div key={item.product_id} className="grid grid-cols-[minmax(0,1fr)_90px_120px_110px_auto] items-center gap-2">
              ...
              />
              <span className="text-sm font-semibold">{brl(item.ordered_qty * item.unit_cost)}</span>
              <Button
                type="button"
                variant="ghost"
                size="icon"
      ...
      <section className="grid gap-2 rounded-lg border border-border bg-card p-4 md:grid-cols-4">
        ...
        <Input placeholder="Frete" inputMode="decimal" value={freight} onChange={(event) => setFreight(event.target.value)} />
        <Input placeholder="Outros" inputMode="decimal" value={other} onChange={(event) => setOther(event.target.value)} />
        <Input placeholder="Desconto" inputMode="decimal" value={discount} onChange={(event) => setDiscount(event.target.value)} />
