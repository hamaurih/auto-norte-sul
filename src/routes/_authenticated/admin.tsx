...
import { adminPageLocation, adminPermissionForPath, visibleModules } from "@/lib/admin-modules";
  ...
  return (
    <Sidebar collapsible="icon" className="border-r-border/70">
      <SidebarHeader className="border-b border-sidebar-border p-3">
        <Link
          to="/admin"
          className="flex min-h-11 items-center gap-3 rounded-lg px-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring"
        >
      ...
      <SidebarContent className="py-2">
        ...
        <div className="px-3 pt-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-sidebar-foreground/45 group-data-[collapsible=icon]:hidden">
          Áreas de trabalho
        </div>
        {modules.map((module) => (
          <Collapsible
            key={module.key}
            open={Boolean(openGroups[module.key])}
            onOpenChange={(open) =>
              setOpenGroups((current) => ({ ...current, [module.key]: open }))
            }
            className="group/module"
          >
            <SidebarGroup className="py-1">
              <SidebarGroupLabel asChild>
                <CollapsibleTrigger className="flex w-full items-center gap-2 rounded-lg px-2 py-1 text-left transition hover:bg-sidebar-accent group-data-[collapsible=icon]:justify-center">
                  <module.icon
                    className={`size-3.5 shrink-0 ${module.accent.split(" ")[1]}`}
                    aria-hidden="true"
                  />
                  ...
                  <ChevronDown
                    className="size-3.5 transition-transform group-data-[state=closed]/module:-rotate-90 group-data-[collapsible=icon]:hidden"
                    aria-hidden="true"
                  />
                </CollapsibleTrigger>
              </SidebarGroupLabel>
      ...
      <SidebarFooter className="border-t border-sidebar-border p-3">
        <div className="flex items-center gap-2 rounded-lg bg-sidebar-accent/60 p-2 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:p-1">
          <span
            className="size-2 shrink-0 rounded-full bg-emerald-500 ring-4 ring-emerald-500/10"
            aria-hidden="true"
          />
          <span className="min-w-0 group-data-[collapsible=icon]:hidden">
            <span className="block truncate text-xs font-semibold">
              {tenant?.name ?? "Norte Sul"}
            </span>
...
function AdminLayout() {
  ...
  const currentPage = adminPageLocation(pathname);
    ...
    return (
      <div className="container-x py-16 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Peça ao administrador para liberar este acesso.
        </p>
      </div>
    );
  }
        ...
        <header className="sticky top-0 z-20 flex h-16 items-center gap-2 border-b border-blue-100/80 bg-white/80 px-3 backdrop-blur-xl md:px-5">
          ...
          <div className="hidden min-w-0 sm:block">
            <p className="truncate text-sm font-semibold">
              {currentPage?.shortcut.label ?? "Visão geral"}
            </p>
            <p className="truncate text-xs text-muted-foreground">
              {currentPage ? `${currentPage.module.title} · ` : ""}
              {tenant?.name ?? "Norte Sul Autopeças"}
            </p>
          </div>
