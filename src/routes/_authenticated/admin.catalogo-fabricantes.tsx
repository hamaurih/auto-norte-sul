...
function Page() {
  ...
  const [source, setSource] = useState({ brandId: "", name: "", sourceKind: "official_site", baseUrl: "", priority: "50" });
  ...
  const addSource = useMutation({ mutationFn: () => saveSourceFn({ data: { ...source, sourceKind: source.sourceKind as any, priority: Number(source.priority) } }), onSuccess: () => { toast.success("Fonte oficial cadastrada"); setSource({ brandId: "", name: "", sourceKind: "official_site", baseUrl: "", priority: "50" }); refresh(); }, onError: (e: Error) => toast.error(e.message) });
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="rounded-lg border bg-card p-4"><h2 className="font-display text-lg font-bold uppercase">Fontes por fabricante</h2>
      ...
      <div className="mt-3 space-y-2">{sources.map(s=><div key={s.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3"><div><div className="font-semibold">{s.brand?.name} · {s.name}</div><div className="text-xs text-muted-foreground">{s.source_kind} · prioridade {s.priority} · {s.allowed_domains?.join(", ")}</div></div><div className="flex gap-2"><a href={s.base_url} target="_blank" rel="noreferrer" className="inline-flex items-center rounded border px-3 py-2 text-xs">Abrir <ExternalLink className="ml-1 h-3 w-3"/></a><Button size="sm" variant="outline" onClick={()=>toggle.mutate({sourceId:s.id,status:s.status==="active"?"paused":"active"})}>{s.status==="active"?<><Pause className="mr-1 h-3 w-3"/>Pausar</>:<><Play className="mr-1 h-3 w-3"/>Ativar</>}</Button></div></div>)}</div>
      <div className="mt-4 grid gap-3 md:grid-cols-5"><BrandSelect value={source.brandId} brands={brands} onChange={v=>setSource({...source,brandId:v})}/><Input placeholder="Nome da fonte" value={source.name} onChange={e=>setSource({...source,name:e.target.value})}/><select className="h-10 rounded-md border bg-background px-3 text-sm" value={source.sourceKind} onChange={e=>setSource({...source,sourceKind:e.target.value})}><option value="official_site">Site oficial</option><option value="official_catalog">Catálogo oficial</option><option value="catalog_api">API oficial</option><option value="supplier_feed">Feed do fornecedor</option><option value="manual">Manual</option></select><Input placeholder="https://fabricante.com.br" value={source.baseUrl} onChange={e=>setSource({...source,baseUrl:e.target.value})}/><Button disabled={!source.brandId||!source.name||!source.baseUrl||addSource.isPending} onClick={()=>addSource.mutate()}><Plus className="mr-1 h-4 w-4"/>Adicionar</Button></div>
    </section>
